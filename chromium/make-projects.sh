...
# produce the necessary R classes and everything will just work.
#
...
# defined by the dependencies of that library. Our solution to this is to have
# a single gradle project that actually includes all of the resources and to
# then have each resource project depend on that project.
#
# The package containing all the actual resources is called `gen.all_resources`,
# and the other resource packages simply depend on it and exist again purely to
# get gradle to generate the right set of R classes to satisfy Chromium.
#
...
get_list_of_standard_R_classes() {
    get_list_of_R_classes | egrep -v 'org.chromium.(base|weblayer/support)'
}
...
make_standard_resource_projs() {
    for R in $(echo "$*"); do
        make_standard_resource_proj $R
    done
}

get_list_of_all_resource_zips() {
    # Exclude resources here
    exclude_pattern="third_party/android|weblayer/(components|public|shell)"

    list=$(find $unpacked_dir/resources -name \*.resources.zip | egrep -v $exclude_pattern)

    # Include these back
    list="$list \
        $unpacked_dir/resources/obj/third_party/android_media/android_media_resources.resources.zip"

    echo $list
}

make_all_resources_proj() {
    package_name="gen.all_resources"
    project_name="all_resources"

    echo "Creating project files for $package_name"

    target_dir=$gen_dir/$project_name
    cp -r templates/all_resources $target_dir

    resource_dirs=""
    for zip in $(get_list_of_all_resource_zips); do
        if [ "$(zipinfo -1 $zip)" = "Empty zipfile." ]; then
            continue
        fi
        outdir=$(echo $zip | cut -d'/' -f3- | cut -d'.' -f1 | sed 's|/|__|g')
...
R_classes=$(get_list_of_standard_R_classes)
make_standard_resource_projs $R_classes

dependencies=""
for R in $(echo $R_classes); do
    path=$(dirname $R)
    project_name=$(echo $path | sed 's|/|__|g')
