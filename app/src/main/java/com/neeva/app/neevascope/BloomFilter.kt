...
class BloomFilter {
    // Filter represents a bloom filter with backing array `data` that will hash each element `k` times.
    ...
    fun loadFilter(fileUrl: Uri) {
        ...
        filter = jsonAdapter.fromJson(fileUrl.toFile().readText()) ?: filter
    }

    /**
     * Check a given key may be a member of the bloom filter
     * @param key to look for in the bloom filter
     * @return true if this bloom filter might contain `key` and false if the key is conclusively not present.
     */
    fun mayContain(key: String): Boolean {
        ...
        val numBits = (filter.dataByteArray.size * 8).toUInt()
        for (i in (0).toUInt() until filter.kUInt) {
            val bit = (h1 % numBits).toInt()
            h1 += h2
            if (filter.dataByteArray[(bit / 8)] and (1 shl (bit % 8)).toByte() == (0).toByte()) {
                return false
            }
        }
        return true
